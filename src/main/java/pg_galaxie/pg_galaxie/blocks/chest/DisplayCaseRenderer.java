...
public class DisplayCaseRenderer<T extends TileEntity> extends TileEntityRenderer<T> {
    ...
    public void render(T tileEntityIn, float partialTicks, MatrixStack matrixStackIn, IRenderTypeBuffer bufferIn, int combinedLightIn, int combinedOverlayIn) {
        ...
        if (block instanceof AbstractChestBlock) {
            ...
            float f1 = icallbackwrapper.<Float2FloatFunction>apply(ChestBlock.getLidRotationCallback((IChestLid) tileEntityIn)).get(partialTicks);
            f1 = 1.0F - f1;
            f1 = 1.0F - f1 * f1 * f1;
            ...
            if(block instanceof DisplayCaseBlock) {
                rendermaterial = this.getMaterial(((DisplayCaseBlock) block).type);
            }else if(tileEntityIn instanceof DisplayCaseTileEntity){
                rendermaterial = this.getMaterial(((DisplayCaseTileEntity)tileEntityIn).type);
            }else{
                return;
            }
            //tileEntityIn, chesttype
    ...
    //}
    private static RenderMaterial getMaterial(PGChestType type) {
        return new RenderMaterial(AtlasTexture.LOCATION_BLOCKS_TEXTURE, type.getTexture());
    }

    //public static final ResourceLocation TEST_CHEST = new ResourceLocation(Pg_galaxie.MODID,"entity/chest/test");
}